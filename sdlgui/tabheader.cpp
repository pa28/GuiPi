...
NAMESPACE_BEGIN(sdlgui)
    ...
    TabHeader::TabButton::TabButton(TabHeader &header, const std::string &label, int icon)
            :
            mHeader(&header), mLabel(label), mIcon(icon) {
    ...
    Vector2i TabHeader::TabButton::preferredSize(SDL_Renderer *ctx) const {
        ...
        if (mIcon) {
            ...
            theme->getUtf8Bounds("icons", 50, iconTxt, &w, &h);
            h = (h * 2) / 3;
        } else {
    ...
    void TabHeader::TabButton::calculateVisibleString(SDL_Renderer *renderer) {
        ...
        std::string displayedText = mHeader->theme()->breakText(renderer, mLabel.c_str(),
                                                                mHeader->theme()->mStandardFont.c_str(),
        ...
        if (displayedText.size() != mLabel.size()) {
            int truncatedWidth = mHeader->theme()->getTextWidth(mHeader->theme()->mStandardFont.c_str(),
                                                                mHeader->fontSize(), displayedText.c_str());
            int dotsWidth = mHeader->theme()->getTextWidth(mHeader->theme()->mStandardFont.c_str(), mHeader->fontSize(),
                                                           dots);
    ...
    void TabHeader::TabButton::drawAtPosition(SDL_Renderer *renderer, const Vector2i &position, bool active) {
        ...
        if (_labelTex.dirty) {
            if (mIcon) {
                auto icon = utf8(mIcon);
                mHeader->mTheme->getTexAndRectUtf8(renderer, _labelTex, 0, 0, icon.data(), "icons",
                                                   50, mHeader->theme()->mTextColor);
            } else {
                std::string lb(mVisibleText.first, mVisibleText.last ? mVisibleText.last - mVisibleText.first : 0xff);

                if (mVisibleText.last != nullptr)
                    lb += dots;
                mHeader->theme()->getTexAndRectUtf8(renderer, _labelTex, 0, 0, lb.c_str(),
                                                    mHeader->theme()->mStandardFont.c_str(),
    ...
    void TabHeader::TabButton::drawActiveBorderAt(SDL_Renderer *renderer, const Vector2i &position,
        ...
        SDL_RenderDrawLine(renderer, xPos + roundToInt(offset), yPos + height + roundToInt(offset),
                           xPos + roundToInt(offset), yPos + roundToInt(offset));
        SDL_RenderDrawLine(renderer, xPos + roundToInt(offset), yPos + roundToInt(offset),
                           xPos + width - roundToInt(offset), yPos + roundToInt(offset));
        SDL_RenderDrawLine(renderer, xPos + width - roundToInt(offset), yPos + roundToInt(offset),
                           xPos + width - roundToInt(offset), yPos + height + roundToInt(offset));
    }
    ...
    void TabHeader::addTab(const std::string &label, int icon) {
        addTab(tabCount(), label, icon);
    }

    void TabHeader::addTab(int index, const std::string &label, int icon) {
        assert(index <= tabCount());
        mTabButtons.insert(std::next(mTabButtons.begin(), index), TabButton(*this, label, icon));
        setActiveTab(index);
    }
    ...
    void TabHeader::drawControls(SDL_Renderer *renderer) {
        ...
        if (_leftIcon.tex) {
            ...
            SDL_RenderCopy(renderer, _leftIcon,
        ...
        if (_rightIcon.tex) {
            ...
            Vector2f rightIconPos = leftControlsPos +
                                    Vector2f(xScaleRight * theme()->mTabControlWidth, yScaleRight * mSize.tofloat().y);
            SDL_RenderCopy(renderer, _rightIcon,
                           Vector2i(rightIconPos.x - _rightIcon.w() / 2, rightIconPos.y - _rightIcon.h() / 2 + 1));
        }
    }
