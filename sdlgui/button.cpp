...
namespace sdlgui {
    ...
    Vector2i Button::preferredSize(SDL_Renderer *ctx) const {
        ...
        float tw = const_cast<Button *>(this)->mTheme->getTextWidth(mTheme->mBoldFont.c_str(), fontSize,
                                                                    mCaption.c_str());
        ...
        if (mIcon) {
            if (nvgIsFontIcon(mIcon)) {
                ih *= 1.5f;
                iw = const_cast<Button *>(this)->mTheme->getUtf8Width("icons", ih, utf8(mIcon).data()) +
                     mSize.y * 0.15f;
            } else {
                int w, h;
                ih *= 0.9f;
    ...
    void Button::draw(SDL_Renderer *renderer) {
        ...
        if (_captionTex.dirty) {
            ...
            mTheme->getTexAndRectUtf8(renderer, _captionTex, 0, 0, mCaption.c_str(), mTheme->mBoldFont.c_str(),
        ...
        if (mIcon) {
            ...
            if (_iconTex.dirty) {
                ...
                if (nvgIsFontIcon(mIcon)) {
                    ih *= 1.5f;
                    mTheme->getTexAndRectUtf8(renderer, _iconTex, 0, 0, icon.data(), "icons", mIconFontSize,
                                              sdlTextColor);
                    iw = _iconTex.w();
                } else {
                    int w, h;
