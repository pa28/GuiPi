...
{
    if (mFont.empty())
        const_cast<std::string&>(mFont) = mTheme->mStandardFont;

    if (mFixedSize != Vector2i::Zero())
        return mFixedSize;

    int w, h;
    const_cast<CheckBox*>(this)->mTheme->getTextBounds(mFont.c_str(), fontSize(), mCaption.c_str(), &w, &h);
    return Vector2i((float)w + 1.7f * (float)fontSize(),  (float)fontSize() * 1.3f);
}
...
void CheckBox::draw(SDL_Renderer *renderer)
{
  ...
  if (_captionTex.dirty)
  {
    ...
    mTheme->getTexAndRectUtf8(renderer, _captionTex, 0, 0, mCaption.c_str(), mFont.c_str(), fontSize(), tColor);
