        ...
        if (mCursorPos > -1) 
        {
            if (mSelectionPos > -1) 
            {
              ...
              float selx = cursorIndex2Position(mSelectionPos, textBound[2], mValueTemp);
                ...
                SDL_Rect sr{ oldDrawPos.x + roundToInt(caretx), oldDrawPos.y + 4, roundToInt(selx - caretx), height() - 4 };
            ...
            if (caretLastTickCount % 1000 < 500)
            {
              ...
              SDL_SetRenderDrawColor(renderer, c.r, c.g, c.b, c.a);
              SDL_RenderDrawLine(renderer, oldDrawPos.x + roundToInt(caretx), oldDrawPos.y + 4,
                oldDrawPos.x + roundToInt(caretx), oldDrawPos.y + roundToInt(lineh) - 3);
            }
        }
    }
}
